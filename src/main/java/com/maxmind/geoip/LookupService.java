 ...
 * <p>
 * <i>Tip:</i> Those deploying the GeoIP API as part of a web application may find it difficult to pass in a File to create the
 * lookup service, as the location of the database may vary per deployment or may even be part of the web-application. In this case,
 * the database should be added to the classpath of the web-app. For example, by putting it into the WEB-INF/classes directory of
 * the web application. The following code snippet demonstrates how to create a LookupService using a database that can be found on
 * the classpath:
 *
 * <pre>
...
public class LookupService
{
        ...
        try {
            ...
            for (int i = 0; i < DATABASE_INFO_MAX_SIZE; i++) {
                ...
                if (delim[0] == 0 && delim[1] == 0 && delim[2] == 0) {
                    ...
                    readBytes(file, dbInfo);
                    // Create the database info object using the string.
                    this.databaseInfo = new DatabaseInfo(new String(dbInfo, Charset.forName("UTF-8")));
                    return databaseInfo;
                }
                file.seek(file.getFilePointer() - 4);
            }
    ...
    public synchronized Location getLocation(long ipnum)
    {
        ...
        try {
            ...
            while (record_buf[record_buf_offset + str_length] != '\0')
                str_length++;
            if (str_length > 0) {
                record.region = new String(record_buf, record_buf_offset, str_length, "ISO-8859-1");
            }
            ...
            while (record_buf[record_buf_offset + str_length] != '\0')
                str_length++;
            if (str_length > 0) {
                record.postalCode = new String(record_buf, record_buf_offset, str_length, "ISO-8859-1");
            }
            record_buf_offset += str_length + 1;
    ...
    private void readBytes(final RandomAccessFile file, final byte[] buf) throws IOException
    {
        readBytes(file, buf, buf.length);
    }

    private void readBytes(final RandomAccessFile file, final byte[] buf, final int len) throws IOException
    {
        int remain = len;
        int count = 0;
        int read = 0;
        while (remain > 0 && (-1 != (read = file.read(buf, count, remain)))) {
            count += read;
            remain -= read;
        }
        if (read == -1) {
            throw new IOException("Hit EOF while reading data!");
        }
    }
}
